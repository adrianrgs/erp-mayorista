...
import { StopSale } from "../../types/producto";
...
// calendario "de un vistazo" con todos los cierres ya registrados.
const expandCommittedDays = (stopSales: StopSale[], propertyId: string): Map<string, string> => {
  const map = new Map<string, string>(); // iso -> id del StopSale que lo cubre
  stopSales.filter(s => s.property_id === propertyId).forEach(s => {
    let cursor = parseISO(s.fechaInicio);
    const end = parseISO(s.fechaFin);
    while (cursor <= end) {
      map.set(toISO(cursor), s.id);
...
// gestionar Stop Sales de un hotel: pinta en rojo todos los días ya cerrados, y permite
// marcar en ámbar días sueltos/salteados (no necesariamente consecutivos) para cerrarlos
// en una sola acción — cada tramo consecutivo se guarda como un registro StopSale aparte.
export default function MultiDayCalendar({
  ...
  const handleDayClick = (iso: string) => {
    ...
    const committedId = committedByDay.get(iso);
    if (committedId) {
      const record = stopSales.find(s => s.id === committedId);
      if (!record) return;
      showConfirm({
        title: "Liberar Stop Sale",
        message: `¿Liberar el cierre completo del ${record.fechaInicio} al ${record.fechaFin}${record.motivo ? ` (${record.motivo})` : ""}?`,
        type: "warning",
        confirmText: "Liberar",
        onConfirm: () => onDeleteStopSale(committedId)
      });
      return;
    }
  ...
  const handleSave = () => {
    ...
    groups.forEach(g => {
      ...
      onAddStopSale({
        ...
        fechaFin: g.fechaFin,
        motivo: motivo || "Corte de ventas contractual"
      });
    });
    ...
    <div className="bg-white border border-zinc-200 rounded-lg p-4 space-y-4">
      ...
      <div className="grid grid-cols-7 gap-1">
        {grid.map((date, i) => {
          ...
          const isCommitted = committedByDay.has(iso);
          ...
          return (
            <button
              ...
              title={isCommitted ? "Clic para liberar este cierre" : "Clic para marcar/desmarcar"}
              className={`h-9 w-full text-xs font-semibold rounded-md transition-colors
                ...
                ${isCommitted ? "bg-red-500 text-white hover:bg-red-600" : ""}
                ${isPending ? "bg-amber-400 text-zinc-900 hover:bg-amber-500" : ""}
              `}
            >
              {date.getDate()}
            </button>
      ...
      <div className="flex flex-wrap items-center gap-3 text-[10px] font-semibold text-zinc-500">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-amber-400 inline-block" /> Selección pendiente</span>
      </div>

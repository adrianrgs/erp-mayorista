...
export default function CobranzasB2BPanel({
  ...
  const [withdrawForm, setWithdrawForm] = useState({
    ...
    reference: "",
    notes: "",
    destino: "cliente" as "cliente" | "empresa"  // reembolso al cliente vs retener a favor de la empresa
  });
  ...
  // resto del panel. Antes sumaba el bruto de las facturas sin netear la NC de una anulación, por lo
  // que una factura anulada seguía apareciendo en la tarjeta.
  ...
  const flashInvoices = (ids: string[]) => {
    ...
    // Scroll a la primera fila encontrada (tras el re-render por selección de cliente y cambio
    // de pestaña), para que el resaltado quede visible aunque estuviera fuera de pantalla.
    if (ids[0]) {
      setTimeout(() => {
  ...
  // (FAC-/NC-/SUP-/…) trae esa factura por su id. En ambos casos resuelve el cliente dueño
  // (por clientId, o por nombre en clientName para cobros B2B legacy sin clientId).
  const buscarPorId = async (rawId: string): Promise<{ clientId: string; invoiceIds: string[] } | null> => {
    ...
    if (!client) return null; // factura(s) existen pero no se pudo resolver la agencia
  ...
  const handleWithdrawSaldoFavor = () => {
    ...
    // Solo el reembolso en efectivo al cliente es una salida de caja (RET-, afecta liquidez). La
    // retención a favor de la empresa NO es egreso: la empresa conserva el efectivo (recuperación).
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 bg-white border border-zinc-200 rounded-lg p-5 space-y-4 shadow-xs">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Filtrar por nombre, RIF..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="Billetera Virtual B2B" disabled={activeClient.saldoFavor <= 0}>
                      Billetera Virtual B2B (Disp: {formatCurrency(activeClient.saldoFavor, getOperatingCurrency())})
                    </option>
                  </select>
                </div>

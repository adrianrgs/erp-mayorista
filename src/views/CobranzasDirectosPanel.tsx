...
interface CobranzasDirectosPanelProps {
...
export default function CobranzasDirectosPanel({
  ...
  const handleVerifyVoucher = (voucher: PaymentVoucher, approve: boolean) => {
    ...
    if (approve) {
      ...
      setStatusMessage(`⚠ Comprobante ${voucher.reference} rechazado. Se notificará al cliente.`);
    }
  ...
  const handleRegisterPaymentSubmit = (e: React.FormEvent) => {
    ...
    if (paymentForm.method !== "Billetera Virtual Directo" && !paymentForm.reference.trim()) {
    ...
    if (paymentForm.method === "Billetera Virtual Directo") {
  ...
  // total). Un voucher por factura bajo un mismo comprobante; la factura queda "Pagado" solo si el
  // abono la salda, si no conserva su estado (queda con saldo pendiente).
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4.5 border border-zinc-200 rounded-lg flex items-center justify-between shadow-xs">
          <div className="space-y-1">
            ...
            <span className="text-2xl font-black block text-zinc-900">{debtorClientsCount} Cliente(s)</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Column: Direct Clients List */}
        <div className="lg:col-span-4 bg-white border border-zinc-200 rounded-lg p-5 space-y-4 shadow-xs">
          <div>
            <h4 className="font-extrabold text-zinc-900 text-xs uppercase tracking-widest">Cartera de Clientes Directos</h4>
              ...
              return (
                ...
                >
                  <div className="space-y-0.5 max-w-[65%]">
                    ...
                    <p className={`text-[9.5px] font-mono ${isSelected ? "text-zinc-400" : "text-zinc-400"}`}>{client.cedula ? `Cédula: ${client.cedula}` : client.telefono}</p>
                  </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          {activeClient ? (
                                ...
                                {clientVouchers.length === 0 ? (
                                  <tr>
                                    <td colSpan={6} className="p-6 text-center text-zinc-400 italic bg-zinc-50/20">
                                      No se registran comprobantes de pago reportados por este cliente.
                                    </td>
                                  </tr>
                                ) : (
            ...
            <div className="bg-white border border-zinc-200 rounded-lg p-10 text-center text-zinc-400 italic shadow-xs">
              No se ha seleccionado ningún cliente directo en la cartera.
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white border border-zinc-200 rounded-lg shadow-xl w-full max-w-lg overflow-hidden animate-fade-in">
            ...
            <div className="bg-zinc-950 text-white px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-[10px] text-zinc-400 font-semibold mt-0.5">
                  Ingrese los datos de la transferencia reportada por {activeClient.nombre}
                </p>
              </div>
              <button 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setPaymentForm(prev => ({
                        ...prev,
                        method: val,
                        reference: val === "Billetera Virtual Directo" ? `WLT-${activeClient.id}-${Date.now().toString().slice(-6)}` : prev.reference,
                        bankName: val === "Billetera Virtual Directo" ? "Billetera Virtual" : prev.bankName
                      }));
                    }}
                  >
                ...
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider block">
                    Nº de Referencia / Aprobación {paymentForm.method !== "Billetera Virtual Directo" && <span className="text-red-500">*</span>}
                  </label>
                  <input
                    type="text"
                    required={paymentForm.method !== "Billetera Virtual Directo"}
                    placeholder={paymentForm.method === "Billetera Virtual Directo" ? "Opcional (billetera)" : "Ej: PM-12984729"}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="border border-zinc-200 bg-linear-to-b from-zinc-50 to-white rounded-lg p-5 shadow-xs relative overflow-hidden font-mono text-xs text-zinc-800 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="text-zinc-400 uppercase text-[9px] font-bold font-sans">Cliente:</span>
